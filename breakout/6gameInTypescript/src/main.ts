...
export class Breakout {
  ...
  initializeGame() {
    ...
    this.gameInterval = setInterval(this._draw, (1000 / this.defaults.framesPerSecond));
  }
  ...
  private _powerupCollision() {
    if (this.state.px + this.defaults.powerUpRadius >= this.state.paddleX &&
  ...
  private _gameOver() {
    ...
    this.ctx.fillText('Click Mouse to Restart', this.canvas.width / 2, this.canvas.height / 2 + 50);
  ...
  private _collisionDetection(x, y) {
    for (var c = 0; c < this.defaults.brickColumnCount; c++) {
      for (var r = 0; r < this.defaults.brickRowCount; r++) {
        ...
        if (b.show) {
          if ((x + this.defaults.ballRadius) >= b.x
            && x - (this.defaults.ballRadius ? this.defaults.ballRadius : 0) <= b.x + this.defaults.brickWidth
            ...
            if (y < b.y + this.defaults.brickHeight + 2) { // if collision occurs on side of brick
            ...
            if (this.state.score === this.defaults.brickRowCount * this.defaults.brickColumnCount * 100) {
              ...
              this.ctx.fillText('Click to go to next stage', this.canvas.width / 2, this.canvas.height / 2 - 20);
              this.ctx.fill();
  ...
  private _drawPowerUp() {
    this.ctx.beginPath();
    this.ctx.arc(this.state.px, this.state.py, this.defaults.powerUpRadius, 0, Math.PI * 2);
    ...
    this.ctx.fillText(this.state.powerUpFalling.symbol, this.state.px, this.state.py);
    this.ctx.closePath();
  }

  private _drawBall() {
    this.ctx.beginPath();
    this.ctx.arc(this.x, this.y, (this.defaults.ballRadius ? this.defaults.ballRadius : 0), 0, Math.PI * 2);
  ...
  private _drawPaddle() {
    this.ctx.beginPath();
    this.ctx.rect(this.state.paddleX, this.canvas.height - this.defaults.paddleHeight, this.state.paddleWidth, this.defaults.paddleHeight);
    ...
    for (var c = 0; c < this.defaults.brickColumnCount; c++) {
      for (var r = 0; r < this.defaults.brickRowCount; r++) {
        if (this.bricks[c][r].show) {
          this.ctx.beginPath();
          this.ctx.rect(this.bricks[c][r].x, this.bricks[c][r].y, (this.defaults.brickWidth ? this.defaults.brickWidth : 0), this.defaults.brickHeight);
  ...
  private _draw() {
    if (!this.state.paused && this.bricks.length) {
      ...
      if (this.y + this.state.dy < (this.defaults.ballRadius ? this.defaults.ballRadius : 0)) {
        this.state.dy = -this.state.dy;
      } else if (this.y + this.state.dy > this.canvas.height - (this.defaults.ballRadius ? this.defaults.ballRadius : 0) - this.defaults.paddleHeight + 2) {
      ...
      if (x + this.state.dx < (this.defaults.ballRadius ? this.defaults.ballRadius : 0) || this.x + this.state.dx > this.canvas.width - (this.defaults.ballRadius ? this.defaults.ballRadius : 0)) {
        this.state.dx = -this.state.dx;
      }

      if (this.state.rightPressed && this.state.paddleX < this.canvas.width - this.state.paddleWidth) {
  ...
  private _initState() {
    this.state = {
      ...
      rightPressed: false,
      leftPressed: false,
      paddleX: (this.canvas.width - ((typeof (this.defaults.paddleWidth) === 'number') ? this.defaults.paddleWidth : 0)) / 2,
      dx: this.defaults.dx,
      dy: this.defaults.dy,
      stage: 1,
      paddleWidth: this.defaults.paddleWidth,
      powerUpCounter: 0,
      powerUpFalling: null,
      px: 5,
      py: 5
    ...
    this.powerups = [
      {
        ...
        endPower: function () {
          this.state.dy = this.state.dy < 0 ? -this.defaults.dy : this.defaults.dy;
          this.state.dx = this.state.dx < 0 ? -this.defaults.dx : this.defaults.dx;
        }
      },
      {
        symbol: '\u25b2', // speed up
    ...
    this.stages = {
      stage1: function () {
        ...
        for (var c = 0; c < this.defaults.brickColumnCount; c++) {
          stage[c] = [];
          for (var r = 0; r < this.defaults.brickRowCount; r++) {
            var brickX = (c * (this.defaults.brickWidth + this.defaults.brickPadding)) + this.defaults.brickOffsetLeft;
            var brickY = (r * (this.defaults.brickHeight + this.defaults.brickPadding)) + this.defaults.brickOffsetTop;
  ...
  private _initDefaults() {
    ...
    this.defaults.brickWidth = (this.canvas.width - (this.defaults.brickOffsetLeft * 2) - (this.defaults.brickColumnCount * this.defaults.brickPadding)) / this.defaults.brickColumnCount;
