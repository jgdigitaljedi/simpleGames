  ...
  getPowerUps(state: IState, defaults: IDefaults) {
    return [
      ...
      },
      {
        ...
        color: '#FF6F00',
        activePower: () => {
          state.paddleWidth = (defaults.paddleWidth ? defaults.paddleWidth : 0) * 1.25;
        },
        endPower: () => {
          state.paddleWidth = defaults.paddleWidth ? defaults.paddleWidth : 0;
        }
      },
      {
        ...
        color: '#C51162',
        activePower: () => {
          state.paddleWidth = (defaults.paddleWidth ? defaults.paddleWidth : 0) * 0.75;
        },
        endPower: () => {
          state.paddleWidth = defaults.paddleWidth ? defaults.paddleWidth : 0;
        }
