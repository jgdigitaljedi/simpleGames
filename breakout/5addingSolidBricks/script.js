(function () {
    ...
    var stage = window.stages['stage' + state.stage]();
    ...
    function collisionDetection(x, y) {
        for (var c = 0; c < defaults.brickColumnCount; c++) {
            for (var r = 0; r < defaults.brickRowCount; r++) {
                var b = stage.bricks[c][r];
                var block = stage.blocks[c][r] || false;
                ...
                if (b && b.show) {
                    if ((x + defaults.ballRadius) >= b.x
                        ...
                        b.show = false;
                        state.score += 100;
                        if (state.score === stage.brickCount * 100) {
                            ...
                            ctx.fill();
                            stage = window.stages['stage' + state.stage]();
                            x = canvas.width/2;
                            y = canvas.height - 30;
                ...
                } else if ((x + defaults.ballRadius) >= block.x
                    && x - defaults.ballRadius <= block.x + block.width
                    && y >= (block.y - defaults.ballRadius)
                    && y <= block.y + defaults.brickHeight + defaults.ballRadius) {
                    if (y < block.y + defaults.brickHeight + 2) { // if collision occurs on side of brick
                        state.dx = -state.dx;
                    } else { // collision occurred on top or bottom
                        state.dy = -state.dy;
                    }
    ...
    function drawBricks() {
        for (var c = 0; c < defaults.brickColumnCount; c++) {
            for (var r = 0; r < defaults.brickRowCount; r++) {
                if (stage.bricks[c].length && stage.bricks[c][r].show) {
                    ctx.beginPath();
                    ctx.rect(stage.bricks[c][r].x, stage.bricks[c][r].y, defaults.brickWidth, defaults.brickHeight);
    ...
    // draws the blocks
    function drawBlocks() {
        stage.blocks.forEach(function (item) {
            if (item.length) {
                item.forEach(function (ite) {
                    ctx.beginPath();
                    ctx.rect(ite.x, ite.y, ite.width, defaults.brickHeight);
                    ctx.fillStyle = defaults.blockColor;
                    ctx.fill();
                    ctx.closePath();
                });
            }
    ...
    function draw() {
        if (!state.paused && stage.bricks.length) {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            if (state.powerUpFalling) {
                drawPowerUp();
                powerupCollision();
